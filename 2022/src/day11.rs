...
#[derive(Clone, Copy)]
enum Op { Add(usize), Mul(usize), AddOld, MulOld }
...
fn parse(input: &str) -> Vec<Monkey> {
    input.split("\n\n").map(|block| {
        ...
        let items = lines[1][18..].split(", ").map(|s| s.parse().unwrap()).collect();
        ...
        Monkey { items, op, div, if_true, if_false, inspections: 0 }
    }).collect()
}
    ...
    for _ in 0..rounds {
        for i in 0..monkeys.len() {
            while let Some(item) = monkeys[i].items.pop_front() {
                ...
                if part1 { worry /= 3; }
                else { worry %= common_divisor; }

                let target = if worry % monkeys[i].div == 0 { monkeys[i].if_true } else { monkeys[i].if_false };
