...
fn parse(input: &str) -> (Vec<Vec<u8>>, Pos, Pos) {
    ...
    let grid = input.lines().enumerate().map(|(r, line)| {
...
fn bfs(grid: &Vec<Vec<u8>>, start: Pos, end: Option<Pos>, find_min: bool) -> usize {
    ...
    while let Some((pos, steps)) = queue.pop_front() {
        if let Some(e) = end { if pos == e { return steps; } }
        else if !find_min && grid[pos.r][pos.c] == 0 { return steps; }
        ...
        for (dr, dc) in &[(0, 1), (0, -1), (1, 0), (-1, 0)] {
            ...
            if nr >= 0 && nr < grid.len() as isize && nc >= 0 && nc < grid[0].len() as isize {
                let npos = Pos { r: nr as usize, c: nc as usize };
                let next_h = grid[npos.r][npos.c];
                if !visited.contains(&npos) && (find_min && next_h <= current_h + 1 || !find_min && current_h <= next_h + 1) {
                    visited.insert(npos);
