...
func buildDisk(input string) [][]bool {
	disk := make([][]bool, 128)
...
func knotHash(input string) string {
	lengths := make([]int, 0)
	for _, c := range input { lengths = append(lengths, int(c)) }
	...
	for i := 0; i < 16; i++ {
		block := 0
		for j := 0; j < 16; j++ { block ^= nums[i*16+j] }
		res += fmt.Sprintf("%02x", block)
	}
	return res
}

func main() { common.Run(2017, 14, Day14{}) }
