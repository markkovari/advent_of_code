...
func (d Day21) solve(input string, iterations int) int {
	rules := d.parse(input)
...
type Day21pattern [][]bool

func (d Day21) count(grid [][]bool) int {
	c := 0
...
func (d Day21) expand(mapping map[uint16][][]bool, grid [][]bool) [][]bool {
	size := len(grid)
	step := 3
	if size%2 == 0 {
		step = 2
...
func (d Day21) newMap(rules []Day21rule) map[uint16][][]bool {
...
func (d Day21) encode(in [][]bool) (out uint16) {
	if len(in) > 2 {
		out |= (1 << 15)
	}
	var i uint
...
func (d Day21) rotate(in [][]bool, n int) [][]bool {
	if n == 0 {
		return in
	}
	size := len(in)
...
func (d Day21) flip(in [][]bool) [][]bool {
	size := len(in)
	out := make([][]bool, size)
	for r := 0; r < size; r++ {
...
type Day21rule struct {
	input, output [][]bool
}

func (d Day21) parse(input string) []Day21rule {
	var rules []Day21rule
	for _, line := range strings.Split(strings.TrimSpace(input), "\n") {
		parts := strings.Split(line, " => ")
		rules = append(rules, Day21rule{input: d.parsePattern(parts[0]), output: d.parsePattern(parts[1])})
	}
	return rules
}

func (d Day21) parsePattern(s string) [][]bool {
