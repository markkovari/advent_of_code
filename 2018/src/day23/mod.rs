...
#[derive(Debug)]
struct Bot { x: i64, y: i64, z: i64, r: i64 }
...
impl Solution for Day23 {
    ...
    fn part1(&self, input: &str) -> Box<dyn Display> {
        let bots: Vec<Bot> = input.lines().map(|line| {
        ...
        let count = bots.iter().filter(|b| 
            (b.x - strongest.x).abs() + (b.y - strongest.y).abs() + (b.z - strongest.z).abs() <= strongest.r
        ).count();
        Box::new(count)
    }
