...
impl Solution for Day03 {
    ...
    fn part1(&self, input: &str) -> Result<String> {
        ...
        let gamma: i32 = counts.iter().enumerate().fold(0, |acc, (i, &c)| {
            if c > (line_count / 2) as i32 {
                acc + (1 << (element_count - 1 - i))
            } else {
                acc
            }
        });
    ...
    F: Fn(char, usize, &Vec<&str>) -> bool,
{
    ...
    for pos in 0..current_lines[0].len() {
        ...
        let next_lines: Vec<&str> = current_lines
            .iter()
            .filter(|l| predicate(l.chars().nth(pos).unwrap(), pos, &current_lines))
            .cloned()
            .collect();
