...
impl TryFrom<&str> for Ingredient {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        ...
        let capacity = caps.name("capacity").unwrap().as_str().parse::<i64>().unwrap();
        let durability = caps.name("durability").unwrap().as_str().parse::<i64>().unwrap();
        let flavor = caps.name("flavor").unwrap().as_str().parse::<i64>().unwrap();
        let texture = caps.name("texture").unwrap().as_str().parse::<i64>().unwrap();
        let calories = caps.name("calories").unwrap().as_str().parse::<i64>().unwrap();
        Ok(Ingredient { name, capacity, durability, flavor, texture, calories })
    }
}
...
fn cookie_calories(ingredients: &[Ingredient], ratios: &[i64]) -> i64 {
    ingredients.iter().enumerate().map(|(idx, ing)| ing.calories * ratios[idx]).sum()
}
...
impl Solution for Day15 {
    ...
    fn part1(&self, input: &str) -> Box<dyn Display> {
        let ingredients: Vec<Ingredient> = input.lines().map(|line| Ingredient::try_from(line).unwrap()).collect();
        if ingredients.len() == 2 {
            ...
            Box::new((0..=100).into_par_iter().flat_map(move |a| {
                let ings = ings.clone();
